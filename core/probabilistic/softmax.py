...
# P1 (v5.1): HY credit-spread factor — positive weight: spread widening -> CSI up.
HY_CENTER = 320.0
W_HY = 0.15
...
def compute_crisis_stress_index(vix: float, dxy: float, ovx: float, hy: Optional[float] = None) -> float:
    """Compute Crisis Stress Index (CSI).

    P1 (v5.1): adds the HY credit-spread factor. `hy` is optional; when None the
    term is omitted (backward compatible).
    """
    vix_term = W_VIX * (vix - VIX_CENTER) / 10.0
    dxy_term = W_DXY * (dxy - DXY_CENTER) / 5.0
    ovx_term = W_OVX * (ovx - OVX_CENTER) / 10.0
    hy_term = W_HY * (hy - HY_CENTER) / 50.0 if hy is not None else 0.0
    return vix_term + dxy_term + ovx_term + hy_term
...
def compute_regime_probabilities(features: FeatureSchema) -> Dict[RegimeName, float]:
    ...
    hy = features.hy_credit_spread if features.hy_credit_spread is not None else HY_CENTER
    csi = compute_crisis_stress_index(vix, dxy, ovx, hy)
