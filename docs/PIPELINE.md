...
- **ADR-001（Accepted, Option B）**：红线触发时编排层将 `phase_for_kernel` 置空，使 kernel 走 HARD_VETO（risk=0），**不改** kernel 内 SAFETY→HARD 顺序；Event/CIO 仍记录 `phase_raw`。同日 sticky day-lock 防 VIX 阈值附近碎步。详见 `docs/adr/001-physical-red-line-vs-safety-gate.md`。
